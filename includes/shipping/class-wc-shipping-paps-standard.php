    ...
    public function calculate_shipping($package = [])
    {
        if (
            ...
            foreach ($package['contents'] as $item_id => $values) {
                ...
                $item_height =  $_product->get_height() ? $_product->get_height() : 1;
                $item_width =  $_product->get_width() ? $_product->get_width() : 1;
                $item_length =  $_product->get_length() ? $_product->get_length() : 1;
                $item_weight = $_product->get_weight() ? $_product->get_weight() : 1;
                $height += $item_height * $quantity;
                $width += $item_width * $quantity;
                $length += $item_length * $quantity;
